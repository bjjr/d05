...
public interface CookRepository extends JpaRepository<Cook, Integer> {
	...
	@Query("select count(m)/count(c)*1.0 from Cook c join c.masterClassesTeach m where m.promoted = true")
	Double findAvgPromotedMasterClasses();
	
	@Query("select count(m)/count(c)*1.0 from Cook c join c.masterClassesTeach m where m.promoted = false")
